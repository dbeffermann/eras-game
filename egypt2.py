...
class Meteor(pygame.sprite.Sprite):
	def __init__(self):
		super().__init__()
		self.image = pygame.image.load(random.choice(["momia2.png","coin2.png"])).convert()
...
while not done:
	...
	for laser in laser_list:
		meteor2_hit_list = pygame.sprite.spritecollide(laser, meteor2_list, True)	
		for meteor2 in meteor2_hit_list:
			all_sprite_list.remove(laser)
			laser_list.remove(laser)
			score_value += 1
			print(score_value)
	...
	"""	
	if not meteor_list and not meteor2_list:
		print('no quedan')
		for i in range(METEORS2):
			meteor2 = Meteor2()
			meteor2.rect.x = random.randrange(20, SCREEN_WIDTH - 20)
			meteor2.rect.y = random.randrange(200) 

		meteor2_list.add(meteor2)
		all_sprite_list.add(meteor2)
		"""
